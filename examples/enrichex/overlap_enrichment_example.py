...
def create_example_gene_sets() -> GeneSetCollection:
    ...
    microglia = GeneSet(
        ...
        genes={
            ...
            "HLA-DRA", "HLA-DRB1", "AIF1", "CX3CR1", "P2RY12"
        },
    ...
    excitatory = GeneSet(
        ...
        genes={
            ...
            "SATB2", "TBR1", "CUX2", "RORB", "FEZF2",
            "FOXP2", "BCL11B", "CRYM", "LPL", "PRSS12"
        },
        source="Lake et al. Nat Biotech 2018",
    ...
    inhibitory = GeneSet(
        ...
        genes={
            ...
            "VIP", "LAMP5", "ADARB2", "LHX6", "NKX2-1",
            "SOX6", "CNR1", "CCK", "TAC1", "RELN"
        },
        source="Lake et al. Nat Biotech 2018",
    ...
    astrocytes = GeneSet(
        ...
        genes={
            ...
            "GJA1", "SLC14A1", "BMPR1B", "MFGE8", "AGT",
            "ATP1A2", "GLUL", "S100B", "ALDH1L1", "FGFR3"
        },
        source="Lake et al. Nat Biotech 2018",
    ...
    oligodendrocytes = GeneSet(
        ...
        genes={
            "MBP", "MOG", "MAG", "PLP1", "MOBP",
            "OLIG1", "OLIG2", "SOX10", "CLDN11", "CNP",
            "ERMN", "GJC2", "ENPP2", "UGT8", "FA2H"
        },
        source="Lake et al. Nat Biotech 2018",
    ...
    endothelial = GeneSet(
        ...
        genes={
            "CLDN5", "FLT1", "PECAM1", "VWF", "NOSTRIN",
            "ITM2A", "SLC2A1", "ABCB1", "ABCG2", "SLC38A5",
            "ADGRL4", "KDR", "ESAM", "CDH5", "TIE1"
        },
        source="Lake et al. Nat Biotech 2018",
    ...
    all_genes = set()
    for gs in [microglia, excitatory, inhibitory, astrocytes, oligodendrocytes, endothelial]:
        all_genes.update(gs.genes)

    # Add some additional common genes to background
    all_genes.update([
        "GAPDH", "ACTB", "TUBB", "APOE", "APP", "MAPT",
        "SNCA", "PSEN1", "PSEN2", "CLU", "CR1", "PICALM",
        "BIN1", "ABCA7", "INPP5D", "MEF2C", "HLA-DRB5",
        "PTK2B", "SORL1", "SLC24A4", "CASS4", "FERMT2"
    ])
...
def main():
    ...
    if len(significant) == 0:
        ...
        for _, row in significant.iterrows():
            logger.info(f"\n{row['gene_set_name']}:")
            logger.info(f"  Overlap: {row['n_overlap']}/{row['n_query']} query genes, "
                       f"{row['n_overlap']}/{row['n_gene_set']} gene set genes")
            logger.info(f"  Odds Ratio: {row['odds_ratio']:.2f} "
                       f"(95% CI: [{row['ci_lower']:.2f}, {row['ci_upper']:.2f}])")
            logger.info(f"  P-value: {row['p_value']:.2e} (adjusted: {row['p_adjusted']:.2e})")

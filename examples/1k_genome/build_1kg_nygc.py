#!/usr/bin/env python3
"""Build a Hail MatrixTable from the NYGC/CCDG 1000 Genomes (2020) VCF files.

The VCF directory should contain the CCDG per-chromosome recalibrated genotype
VCFs.  Annotated VCFs (*.annotated.*) and "others" contig files are excluded
automatically.  Only ``*_chr*.recalibrated_variants.vcf.gz`` files are staged.

Example usage::

    # Basic
    python build_1kg_nygc.py --vcf-dir /data/1kg/vcfs --output-mt /data/1kg/out.mt

    # With chromosomes and sample annotations
    python build_1kg_nygc.py \\
...
def main(
    ...
    chrom_list = None
    if chromosomes:
        chrom_list = [c.strip() for c in chromosomes.split(",") if c.strip()]

    # -- Banner --
    logger.info("=" * 42)
    logger.info("  1000G NYGC MatrixTable Build")

...
def plot_runtime_vs_cpus(df: pd.DataFrame, output_path: Path):
    ...
    # Plot actual data
    ax.plot(cpu_counts, total_times, 'o-', linewidth=2.5, markersize=12,
            color='#2c3e50', label='Measured Runtime', zorder=3)
    ...
    ax.plot(cpu_counts, ideal_times, '--', linewidth=2,
            color='#27ae60', alpha=0.7, label='Ideal Linear Scaling', zorder=2)
    ...
    ax.set_title('HGC Workflow Runtime vs CPU Count (Strong Scaling)',
    ...
    ax.text(0.02, 0.98, f'Fixed Cohort: {sample_size} samples',
...
def plot_speedup(df: pd.DataFrame, output_path: Path):
    ...
    # Plot actual speedup
    ax.plot(cpu_counts, speedups, 'o-', linewidth=2.5, markersize=12,
            color='#e74c3c', label='Measured Speedup', zorder=3)

    # Plot ideal linear speedup
    ax.plot(cpu_counts, ideal_speedup, '--', linewidth=2,
            color='#27ae60', alpha=0.7, label='Ideal Linear Speedup', zorder=2)

    ax.set_xlabel('Number of CPU Cores', fontsize=14, fontweight='bold')
    ax.set_ylabel('Speedup (×)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Speedup vs CPU Count',
                fontsize=16, fontweight='bold', pad=20)
    ...
    # Add sample size annotation
    sample_size = df['sample_size'].iloc[0]
    ax.text(0.98, 0.02, f'Fixed Cohort: {sample_size} samples\nBaseline: {baseline_cpus} CPUs',
...
def plot_efficiency(df: pd.DataFrame, output_path: Path):
    ...
    # Plot efficiency
    ax.plot(cpu_counts, efficiencies, 'o-', linewidth=2.5, markersize=12,
            color='#3498db', label='Parallel Efficiency', zorder=3)

    # Add 100% efficiency reference line
    ax.axhline(y=100, color='#27ae60', linestyle='--', linewidth=2,
              alpha=0.7, label='Ideal Efficiency (100%)', zorder=2)

    # Add 50% efficiency reference line
    ax.axhline(y=50, color='#f39c12', linestyle=':', linewidth=1.5,
              alpha=0.5, label='50% Efficiency', zorder=1)

    ax.set_xlabel('Number of CPU Cores', fontsize=14, fontweight='bold')
    ax.set_ylabel('Parallel Efficiency (%)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Parallel Efficiency vs CPU Count',
                fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='upper right', fontsize=12, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 110)
...
def plot_step_breakdown(df: pd.DataFrame, output_path: Path):
    ...
    for step, label, color, marker in zip(steps, step_labels, colors, markers):
        ...
        ax.plot(cpu_counts, times, marker=marker, linewidth=2.5, markersize=10,
               color=color, label=label, alpha=0.85)

    ax.set_xlabel('Number of CPU Cores', fontsize=14, fontweight='bold')
    ax.set_ylabel('Runtime (seconds)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Step-by-Step Scaling',
                fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='upper right', fontsize=12, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3)
...
def plot_comprehensive_scaling(df: pd.DataFrame, output_path: Path):
    ...
    ax1.plot(cpu_counts, total_times, 'o-', linewidth=2.5, markersize=10,
            color='#2c3e50', label='Measured', zorder=3)
    ax1.plot(cpu_counts, ideal_times, '--', linewidth=2,
            color='#27ae60', alpha=0.7, label='Ideal', zorder=2)
    ...
    # Panel 2: Speedup
    ax2.plot(cpu_counts, speedups, 'o-', linewidth=2.5, markersize=10,
            color='#e74c3c', label='Measured', zorder=3)
    ax2.plot(cpu_counts, ideal_speedup, '--', linewidth=2,
            color='#27ae60', alpha=0.7, label='Ideal', zorder=2)
    ...
    # Panel 3: Efficiency
    ax3.plot(cpu_counts, efficiencies, 'o-', linewidth=2.5, markersize=10,
    ...
    for step, label, color, marker in zip(steps, step_labels, colors, markers):
        times = df[step].values
        ax4.plot(cpu_counts, times, marker=marker, linewidth=2, markersize=8,
               color=color, label=label, alpha=0.85)
    ...
    # Add overall title
    sample_size = df['sample_size'].iloc[0]
    fig.suptitle(f'HGC Workflow CPU Scaling Analysis (Fixed Cohort: {sample_size} samples)',
...
def generate_summary_table(df: pd.DataFrame, output_path: Path):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"CPU Counts Tested: {', '.join(map(str, df['num_cpus'].tolist()))}\n\n")

        f.write("-"*80 + "\n")
        f.write(f"{'CPUs':>6} {'Total(s)':>10} {'Speedup':>10} {'Efficiency':>12} "
               f"{'GVCF→VDS':>12} {'VDS→MT':>10} {'QC':>10} {'MT→VCF':>10}\n")
        f.write("-"*80 + "\n")

        for _, row in df.iterrows():
            f.write(f"{row['num_cpus']:>6.0f} "
        ...
        f.write("KEY METRICS:\n")
        f.write(f"  Maximum Speedup: {max_speedup:.2f}× at {max_speedup_cpus:.0f} CPUs\n")
        f.write(f"  Best Efficiency: {best_efficiency:.1f}% at {best_eff_cpus:.0f} CPUs\n")
        ...
        f.write(f"  Time Saved (baseline vs fastest): {time_saved:.0f}s ({percent_saved:.1f}%)\n")
...
def main():
    ...
    try:
        ...
        plot_comprehensive_scaling(df, args.results_dir / "cpu_scaling_comprehensive.png")

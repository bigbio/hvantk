...
def plot_runtime_breakdown(timing_df: pd.DataFrame, output_path: Path):
    ...
    for step, label, color in zip(steps, step_labels, colors):
        ...
        ax.bar(sample_sizes, values, bottom=bottom, label=label, color=color, alpha=0.85)
        bottom += values

    ax.set_xlabel('Sample Size', fontsize=14, fontweight='bold')
    ax.set_ylabel('Runtime (seconds)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Runtime Breakdown by Sample Size', fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='upper left', fontsize=12, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3, axis='y')

    # Add value labels on bars
    for i, (size, total) in enumerate(zip(sample_sizes, timing_df['total_sec'])):
        ax.text(size, total + total * 0.02, f'{total:.0f}s',
...
def plot_total_runtime(timing_df: pd.DataFrame, output_path: Path):
    ...
    # Plot actual data
    ax.plot(sample_sizes, total_times, 'o-', linewidth=2.5, markersize=10,
            color='#2c3e50', label='Actual Runtime', zorder=3)

    # Fit linear trend
    z_linear = np.polyfit(sample_sizes, total_times, 1)
    p_linear = np.poly1d(z_linear)
    ax.plot(sample_sizes, p_linear(sample_sizes), '--', linewidth=2,
            color='#e74c3c', alpha=0.7, label=f'Linear Fit: y = {z_linear[0]:.2f}x + {z_linear[1]:.0f}')
    ...
    ax.set_title('HGC Workflow Total Runtime vs Sample Size', fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='upper left', fontsize=11, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3)

    # Add R-squared annotation
    ax.text(0.98, 0.05, f'$R^2$ = {r_squared:.4f}',
...
def plot_memory_usage(memory_df: pd.DataFrame, output_path: Path):
    ...
    # Plot memory usage
    ax.plot(sample_sizes, memory_gb, 'o-', linewidth=2.5, markersize=10,
            color='#9b59b6', label='Peak Memory Usage', zorder=3)

    # Fit linear trend
    if len(sample_sizes) > 1:
        z = np.polyfit(sample_sizes, memory_gb, 1)
        p = np.poly1d(z)
        ax.plot(sample_sizes, p(sample_sizes), '--', linewidth=2,
                color='#e74c3c', alpha=0.7, label=f'Linear Fit: y = {z[0]:.4f}x + {z[1]:.2f}')

    ax.set_xlabel('Sample Size', fontsize=14, fontweight='bold')
    ax.set_ylabel('Peak Memory Usage (GB)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Memory Usage vs Sample Size', fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='upper left', fontsize=11, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3)
...
def plot_step_comparison(timing_df: pd.DataFrame, output_path: Path):
    ...
    for label, (col, color) in steps.items():
        ax.plot(sample_sizes, timing_df[col], 'o-', linewidth=2, markersize=8,
                color=color, label=label, alpha=0.85)

    ax.set_xlabel('Sample Size', fontsize=14, fontweight='bold')
    ax.set_ylabel('Runtime (seconds)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Step-by-Step Runtime Comparison', fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='upper left', fontsize=12, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3)
...
def plot_scaling_efficiency(timing_df: pd.DataFrame, output_path: Path):
    ...
    ax.plot(sample_sizes, time_per_sample, 'o-', linewidth=2.5, markersize=10,
            color='#16a085', label='Time per Sample', zorder=3)

    ax.set_xlabel('Sample Size', fontsize=14, fontweight='bold')
    ax.set_ylabel('Time per Sample (seconds)', fontsize=14, fontweight='bold')
    ax.set_title('HGC Workflow Scaling Efficiency', fontsize=16, fontweight='bold', pad=20)
    ax.legend(loc='best', fontsize=11, frameon=True, shadow=True)
    ax.grid(True, alpha=0.3)

    # Add value labels
    for size, tps in zip(sample_sizes, time_per_sample):
        ax.annotate(f'{tps:.2f}s',
                   xy=(size, tps),
                   xytext=(0, 10),
                   textcoords='offset points',
                   ha='center',
    ...
    ax.axhline(y=mean_tps, color='red', linestyle='--', alpha=0.5,
...
def generate_summary_report(timing_df: pd.DataFrame, memory_df: pd.DataFrame, output_path: Path):
    ...
    for _, row in timing_df.iterrows():
        ...
        total = row['total_sec']
        report_lines.append(f"Sample Size: {size:4d} | Total Runtime: {total:8.1f}s ({total/60:6.1f}min)")
    ...
    for i in range(len(sample_sizes) - 1):
        ...
        report_lines.append(f"{sample_sizes[i]:4.0f} → {sample_sizes[i+1]:4.0f} samples "
    ...
    if len(valid_memory) > 0:
        ...
        for _, row in valid_memory.iterrows():
            ...
            report_lines.append(f"Sample Size: {size:4d} | Peak Memory: {mem_gb:6.1f}GB "
...
def main():
    ...
    # Generate all plots
    plot_runtime_breakdown(timing_df, plots_dir / f"{args.output_prefix}_runtime_breakdown.png")
    plot_total_runtime(timing_df, plots_dir / f"{args.output_prefix}_total_runtime.png")
    plot_step_comparison(timing_df, plots_dir / f"{args.output_prefix}_step_comparison.png")
    plot_scaling_efficiency(timing_df, plots_dir / f"{args.output_prefix}_scaling_efficiency.png")

    if memory_df['peak_memory_mb'].notna().any():
        plot_memory_usage(memory_df, plots_dir / f"{args.output_prefix}_memory_usage.png")

...
from hvantk.algorithms.rerank.catalog.profile import DiseaseProfile
from hvantk.algorithms.rerank.catalog.registry import (
    axis,
    AXES,
    build_config,
    _constraint_first,
...
def test_constraint_first_reorders():
    feats = [
        FeatureAxis("expression", lambda: pd.DataFrame({"gene": ["A"], "x": [1.0]})),
...
def test_build_config_string_and_override():
    ...
    try:
        ...
        override = FeatureAxis(
            "expression", lambda: pd.DataFrame({"gene": ["A", "B"], "x": [3.0, 4.0]})
        )
...
def test_axis_registry_decorator():
    ...
    try:
        ...
        def _build(profile):
            return FeatureAxis(
                "test_synthetic", lambda: pd.DataFrame({"gene": ["X"], "v": [1.0]})
            )

"""
Test suite for HGC QC computation and the static HTML QC report.

This module contains tests to ensure QC computation, table export, and the
static HTML triage report work correctly. Interactive/plotly plotting and
the per-metric ``QCMetrics.plot_*`` methods were retired; plot QC metrics
from the tables (``get_sample_metrics_df`` / ``get_variant_metrics_df``)
...
def test_html_qc_report_generation(test_mt):
    """Test 3: Generate comprehensive HTML QC reports."""
    from hvantk.algorithms.hgc import compute_full_qc
...
def test_qc_data_validation(test_mt):
    """Test 4: Validate QC data quality and structure."""
    from hvantk.algorithms.hgc import compute_full_qc

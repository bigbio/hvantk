...
class GeneDiseaseTableStreamer(HailDataStreamer):
    ...
    * ``source_name``  – e.g. ``"ClinGen"`` or ``"GenCC"``
    * ``classification_levels`` – ordered list strongest → weakest
    * ``annotation_prefix`` – e.g. ``"clingen_"`` or ``"gencc_"``
    """

    # --- Subclass properties (override in subclasses) ---
    source_name: str = ""
    ...
    def get_geneset_per_disease(
        ...
        if self._keying_mode in self._PER_ASSERTION_MODES:
            ...
            if include_mondo_id:
                ...
                if "disease_mondo_pairs" not in row_fields:
                    raise ValueError(
                        ...
                        f"{self.source_name} tables. Rebuild with the appropriate "
                        "table builder (key_by='gene') from a recent hvantk version."
                    )
            ...
            disease_to_genes_flat: Dict[str, Set[str]] = {}
            for row in rows:
                for disease in row.disease_labels or []:
                    if disease not in disease_to_genes_flat:
                        disease_to_genes_flat[disease] = set()
                    disease_to_genes_flat[disease].add(row.gene_symbol)
            return disease_to_genes_flat
    ...
    def categorize_by_ontology(
        ...
        if self._keying_mode in self._PER_ASSERTION_MODES:
            ...
            if "disease_mondo_pairs" not in row_fields:
                raise ValueError(
                    f"Gene-keyed {self.source_name} tables must include "
                    "disease_mondo_pairs to categorize by ontology. Rebuild with "
                    "the appropriate table builder (key_by='gene') from a recent "
                    "hvantk version."
                )
            rows = []
    ...
    def get_genes_by_ontology_category(
        ...
        if self._keying_mode in self._PER_ASSERTION_MODES:
            ...
            if "disease_mondo_pairs" not in row_fields:
                raise ValueError(
                    f"Gene-keyed {self.source_name} tables must include "
                    "disease_mondo_pairs to query ontology categories. Rebuild with "
                    "the appropriate table builder (key_by='gene') from a recent "
                    "hvantk version."
                )
            rows = ht.select("gene_symbol", "disease_mondo_pairs").collect()
    ...
    def compute_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            f"{self._grouping_field_short}_counts": grouping_counts,
    ...
    def describe(self) -> str:
        ...
        lines = [
            f"{self.source_name} Gene-Disease Validity Dataset",
            "=" * (len(self.source_name) + 34),
    ...
    def annotate_gene_table(
        ...
        if annotation_fields is None:
            mapping = {
                f"{prefix}classifications": "classifications",
                f"{prefix}diseases": "disease_labels",
                f"{prefix}max_classification": "max_classification_label",
                f"{prefix}n_diseases": "n_diseases",
            }
        else:
            mapping = {f"{prefix}{field}": field for field in annotation_fields}

        annotations = {}
        for out_field, in_field in mapping.items():
            annotations[out_field] = gene_level_ht[join_expr][in_field]
    ...
    def export_for_enrichex(
        ...
        source_key = self.source_name.lower().replace(" ", "_")
        collection = load_gene_sets_from_dict(
            {k: sorted(v) for k, v in gene_sets.items()},
            source=f"{source_key}_gene_disease_validity",
        )
    ...
    def validate_table(self) -> bool:
        """Validate that the loaded table has expected schema."""
        self._ensure_table_loaded()
        ht = self._table
        if ht is None:
            raise ValueError(f"{self.source_name} table not loaded.")
        ...
        if self._keying_mode in self._PER_ASSERTION_MODES:
            ...
            } | self._gene_disease_required_fields
            ...
            raise ValueError(
                f"{self.source_name} table has unsupported keying. "
                "Expected key_by gene_disease or gene."
            )

        missing = sorted(required - row_fields)
        if missing:
            raise ValueError(
                f"{self.source_name} table missing required fields: "
                f"{', '.join(missing)}"
            )
    ...
    def _gene_disease_required_fields(self) -> Set[str]:
        """Additional required fields for gene_disease keying mode.

        Override in subclasses to add source-specific required fields.
        """
        return set()

    @property
    def _grouping_field(self) -> str:
        """Field name for organizational grouping (used by compute_stats).
    ...
    def _date_field(self) -> Optional[str]:
        """Date field name for last_update stat.

        Override in subclasses. Return None if no date field exists.
        """
        return None
    ...
    def _ensure_table_path_exists(self) -> None:
        ...
        raise FileNotFoundError(
            f"{self.source_name} table path not found: {self.table_path}"
        )
    ...
    def _classification_level(self, classification: str) -> int:
        return self.classification_levels.index(classification)
    ...
    def _normalize_classification(self, classification: str) -> str:
        ...
        lookup = {c.lower(): c for c in self.classification_levels}
        key = normalized.lower()
        if key not in lookup:
            raise ValueError(
                f"Invalid classification '{classification}'. "
                f"Expected one of {self.classification_levels}."
            )
        return lookup[key]
    ...
    def _write_gmt(self, output_path: str, gene_sets: Dict[str, Set[str]]) -> None:

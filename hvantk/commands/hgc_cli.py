...
@hgc_group.command(name="qc-batch")
@click.option('--input-pattern', '-p', required=True, help='Pattern for input MatrixTable files (e.g., "*.mt")')
@click.option('--output-dir', '-o', required=True, help='Output directory for all QC results')
@click.option('--compute-qc', is_flag=True, help='Compute QC metrics first')
@click.option('--generate-plots', is_flag=True, help='Generate individual plots for each dataset')
@click.option('--generate-reports', is_flag=True, help='Generate HTML reports for each dataset')
...
@click.option('--style',
              ...
              default='default',
              help='Plot style')
@click.option('--parallel', '-j', default=1, help='Number of parallel processes')
@click.option('--dry-run', is_flag=True, help='Show what would be done without executing')
@click.pass_context
def qc_batch(ctx, input_pattern, output_dir, compute_qc, generate_plots, generate_reports,
             plot_format, style, parallel, dry_run):
    """
    Batch process multiple MatrixTables for QC analysis.

    Process multiple datasets in batch mode, computing QC metrics, generating plots,
    and creating HTML reports for each dataset.

    Examples:
        hvantk hgc qc-batch -p "cohort_*.mt" -o batch_qc/ --compute-qc --generate-reports
        hvantk hgc qc-batch -p "data/*.mt" -o results/ --generate-plots --generate-reports
        hvantk hgc qc-batch -p "*.mt" -o qc/ --compute-qc --parallel 4 --dry-run
    """
    try:
        import glob
        from pathlib import Path
        ...
        if dry_run:
            ...
            click.echo(f"   • Found files: {len(input_files)}")
            for f in input_files[:5]:  # Show first 5
                click.echo(f"     - {f}")
            if len(input_files) > 5:
                click.echo(f"     ... and {len(input_files) - 5} more")
            click.echo(f"   • Output directory: {output_path.absolute()}")
            click.echo(f"   • Compute QC: {compute_qc}")
            click.echo(f"   • Generate plots: {generate_plots}")
            click.echo(f"   • Generate reports: {generate_reports}")
        ...
        def process_single_mt(input_file):
            """Process a single MatrixTable."""
            try:
                import hail as hl
                ...
                click.echo(f"🔄 Processing {file_stem}...")

                # Initialize Hail (each process needs its own)
                hl.init(quiet=True)

                # Load MatrixTable
                mt = hl.read_matrix_table(input_file)

                # Compute QC if requested
                if compute_qc:
                    qc_results = compute_full_qc(mt)
                else:
                    # Check for existing QC
                    if 'sample_qc' not in mt.col and 'variant_qc' not in mt.row:
                        click.echo(f"⚠️  No QC annotations found in {file_stem}, skipping")
                        return None
                ...
                results = {'file': file_stem, 'success': True}

                # Generate plots if requested
                if generate_plots:
                    plots_dir = output_path / file_stem / 'plots'
                    plots_dir.mkdir(parents=True, exist_ok=True)

                    plot_kwargs = {'style': style, 'figsize': (10, 6)}

                    if qc_results.has_sample_qc:
                        qc_results.plot_sample_overview(
                            save_path=plots_dir / f'sample_overview.{plot_format}',
                            **plot_kwargs
                        )

                    if qc_results.has_variant_qc:
                        qc_results.plot_variant_overview(
                            save_path=plots_dir / f'variant_overview.{plot_format}',
                            **plot_kwargs
                        )
                ...
                if generate_reports:
                    report_path = output_path / file_stem / f'{file_stem}_qc_report.html'
                    report_path.parent.mkdir(parents=True, exist_ok=True)

                    qc_results.generate_html_report(
                        output_path=report_path,
                        title=f"QC Report: {file_stem}"
                    )
        ...
        # Process files
        if parallel > 1:
            click.echo(f"🚀 Processing {len(input_files)} files with {parallel} parallel processes...")
            with concurrent.futures.ProcessPoolExecutor(max_workers=parallel) as executor:
                results = list(executor.map(process_single_mt, input_files))
        else:
            click.echo(f"🚀 Processing {len(input_files)} files sequentially...")
            results = [process_single_mt(f) for f in input_files]
        ...
        if failed:
            ...
            for result in failed:
                click.echo(f"   • {result['file']}: {result.get('error', 'Unknown error')}")

        if successful:
            click.echo(f"\n✅ Results saved in: {output_path.absolute()}")

    except Exception as e:
        logger.exception(f"Batch QC processing failed: {e}")
        click.echo(f"❌ Error: {e}", err=True)
        ctx.exit(1)

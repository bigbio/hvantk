...
QC_THRESHOLDS = {
    ...
    'ti_tv_ratio': {'good_min': 1.8, 'good_max': 2.2, 'acceptable_min': 1.5, 'acceptable_max': 2.5},
    ...
    for col in df.columns:
        if isinstance(col, str) and '.' in col:
            ...
            if base_name in seen_names:
                ...
                new_columns[prev_idx] = f"{df.columns[prev_idx].split('.')[-2]}_{base_name}"
...
def _prepare_variant_qc_data(variant_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'AC' in df.columns:
        ...
        try:
            df['AC_alt'] = df['AC'].apply(lambda x: x[1] if isinstance(x, (list, np.ndarray)) and len(x) > 1 else x)
    ...
    if 'AF' in df.columns:
        ...
        try:
            df['AF_alt'] = df['AF'].apply(lambda x: x[1] if isinstance(x, (list, np.ndarray)) and len(x) > 1 else x)
        except (TypeError, IndexError):
...
def _add_threshold_lines(ax, metric: str, thresholds: Optional[Dict] = None,
    ...
    if 'good' in thresholds:
        line_func(thresholds['good'], color=QC_COLORS['pass'],
                 linestyle='--', alpha=0.7, label='Good threshold')

    if 'acceptable' in thresholds:
        line_func(thresholds['acceptable'], color=QC_COLORS['warn'],
                 linestyle='--', alpha=0.7, label='Acceptable threshold')

    if 'fail' in thresholds:
        line_func(thresholds['fail'], color=QC_COLORS['fail'],
                 linestyle='--', alpha=0.7, label='Fail threshold')

    # Handle range thresholds (e.g., Ti/Tv ratio)
    if 'good_min' in thresholds and 'good_max' in thresholds:
        line_func(thresholds['good_min'], color=QC_COLORS['pass'],
                 linestyle='--', alpha=0.7)
        line_func(thresholds['good_max'], color=QC_COLORS['pass'],
                 linestyle='--', alpha=0.7, label='Good range')


def plot_sample_call_rate_distribution(sample_df: pd.DataFrame,
    ...
    **kwargs
    ...
    # Plot histogram
    n, bins_array, patches = ax.hist(call_rates, bins=bins,
                                   alpha=0.7, color=QC_COLORS['neutral'],
                                   edgecolor='black', linewidth=0.5)

    # Add KDE if requested and seaborn is available
    if kde and HAS_SEABORN:
        try:
            sns.kdeplot(data=call_rates, ax=ax, color='red', linewidth=2, warn_singular=False)
    ...
    if outliers:
        ...
        threshold = thresholds.get('acceptable', default_thresholds.get('acceptable', 0.85)) if thresholds else default_thresholds.get('acceptable', 0.85)

        outlier_mask = call_rates < threshold
        if outlier_mask.any():
            outlier_samples = call_rates[outlier_mask]
            ax.hist(outlier_samples, bins=bins_array, alpha=0.8,
                   color=QC_COLORS['fail'], label=f'Low call rate (n={len(outlier_samples)})')

    # Customize plot
    ax.set_xlabel('Sample Call Rate', fontsize=12)
    ax.set_ylabel('Number of Samples', fontsize=12)
    ax.set_title(f'Sample Call Rate Distribution (n={len(call_rates)})', fontsize=14, fontweight='bold')
...
def plot_sample_titv_distribution(sample_df: pd.DataFrame,
                                 thresholds: Optional[Dict] = None,
                                 bins: int = 50,
                                 kde: bool = True,
                                 style: str = 'default',
    ...
    **kwargs
    ...
    # Plot histogram
    ax.hist(filtered_ratios, bins=bins, alpha=0.7, color=QC_COLORS['neutral'],
           edgecolor='black', linewidth=0.5)

    # Add KDE if requested
    if kde and HAS_SEABORN:
        try:
            sns.kdeplot(data=filtered_ratios, ax=ax, color='red', linewidth=2, warn_singular=False)
        except Exception as e:
            logger.warning(f"Could not add KDE overlay: {e}")
    ...
    ax.set_ylabel('Number of Samples', fontsize=12)
    ax.set_title(f'Sample Ti/Tv Ratio Distribution (n={len(ti_tv_ratios)})', fontsize=14, fontweight='bold')
...
def plot_sample_depth_distribution(sample_df: pd.DataFrame,
                                  bins: int = 50,
                                  kde: bool = True,
    ...
    **kwargs
    ...
    if depth_col is None:
        raise ValueError("Sample DataFrame must contain depth statistics (mean depth column)")
    ...
    # Plot histogram
    ax.hist(depths, bins=bins, alpha=0.7, color=QC_COLORS['neutral'],
           edgecolor='black', linewidth=0.5)

    # Add KDE if requested
    if kde and HAS_SEABORN:
        try:
            sns.kdeplot(data=depths, ax=ax, color='red', linewidth=2, warn_singular=False)
        except Exception as e:
            logger.warning(f"Could not add KDE overlay: {e}")
    ...
    ax.set_ylabel('Number of Samples', fontsize=12)
    ax.set_title(f'Sample Depth Distribution (n={len(depths)})', fontsize=14, fontweight='bold')
...
def plot_sample_qc_overview(sample_df: pd.DataFrame,
                           thresholds: Optional[Dict] = None,
                           style: str = 'default',
    ...
    **kwargs
    ...
    if available_cols:
        ...
        ax4.pie(genotype_data.values, labels=genotype_data.index, colors=colors[:len(genotype_data)], autopct='%1.1f%%')
        ax4.set_title('Genotype Distribution')
    else:
        ax4.text(0.5, 0.5, 'Genotype Counts\nNot Available', ha='center', va='center', transform=ax4.transAxes)
...
def plot_variant_call_rate_distribution(variant_df: pd.DataFrame,
                                       thresholds: Optional[Dict] = None,
                                       bins: int = 50,
                                       kde: bool = True,
                                       style: str = 'default',
    ...
    **kwargs
    ...
    # Plot histogram
    ax.hist(call_rates, bins=bins, alpha=0.7, color=QC_COLORS['neutral'],
           edgecolor='black', linewidth=0.5)
    ...
    ax.set_title(f'Variant Call Rate Distribution (n={len(call_rates)})', fontsize=14, fontweight='bold')
...
def plot_allele_frequency_spectrum(variant_df: pd.DataFrame,
                                  bins: int = 50,
                                  log_scale: bool = True,
                                  style: str = 'default',
                                  figsize: Tuple[float, float] = (10, 6),
    ...
    **kwargs
    ...
    if af_col is None:
        raise ValueError("Variant DataFrame must contain allele frequency column (AF or AF_alt)")

    # Get allele frequencies, handling both single values and arrays
    if af_col == 'AF':
        # Try to extract alternate allele frequency from array
        try:
            afs = df[af_col].apply(lambda x: x[1] if isinstance(x, (list, np.ndarray)) and len(x) > 1 else x)
        except:
            afs = df[af_col]
    else:
        afs = df[af_col]
    ...
    # Plot histogram
    ax.hist(afs_filtered, bins=bins, alpha=0.7, color=QC_COLORS['neutral'],
           edgecolor='black', linewidth=0.5)
    ...
    ax.set_ylabel('Number of Variants', fontsize=12)
    ax.set_title(f'Allele Frequency Spectrum (n={len(afs_filtered)} polymorphic)', fontsize=14, fontweight='bold')
...
def plot_hwe_pvalues(variant_df: pd.DataFrame,
                    thresholds: Optional[Dict] = None,
    ...
    **kwargs
    ...
    # Plot histogram
    ax.hist(plot_values, bins=bins, alpha=0.7, color=QC_COLORS['neutral'],
           edgecolor='black', linewidth=0.5)
    ...
    if thresholds:
        for name, threshold in thresholds.items():
            ...
            ax.axvline(line_val, color=color, linestyle='--', alpha=0.7,
                      label=f'{name.title()}: {threshold:.0e}')

    # Customize plot
    ax.set_xlabel(xlabel, fontsize=12)
    ax.set_ylabel('Number of Variants', fontsize=12)
    ax.set_title(f'Hardy-Weinberg Equilibrium p-values {title_suffix} (n={len(pvalues)})',
                fontsize=14, fontweight='bold')

    # Add statistics
    if log_transform:
        stats_text = f'Mean: {plot_values.mean():.2f}\nMedian: {plot_values.median():.2f}'
    else:
        stats_text = f'Mean: {plot_values.mean():.2e}\nMedian: {plot_values.median():.2e}'
...
def plot_variant_qc_overview(variant_df: pd.DataFrame,
                           thresholds: Optional[Dict] = None,
                           style: str = 'default',
                           figsize: Tuple[float, float] = (15, 10),
                           save_path: Optional[Union[str, Path]] = None,
    ...
    **kwargs
    ...
    if af_col in df.columns:
        ...
        if len(afs_filtered) > 0:
            ax2.hist(afs_filtered, bins=30, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
        ax2.set_xlabel('Allele Frequency')
        ax2.set_ylabel('Count')
        ax2.set_title('AF Spectrum')
    else:
        ax2.text(0.5, 0.5, 'Allele Frequency\nNot Available', ha='center', va='center', transform=ax2.transAxes)
    ...
    if 'p_value_hwe' in df.columns:
        ...
        if len(pvals) > 0:
            ...
            ax3.hist(log_pvals, bins=30, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
            _add_threshold_lines(ax3, 'hwe_pvalue', thresholds)
        ax3.set_xlabel('-log10(HWE p-val)')
        ax3.set_ylabel('Count')
        ax3.set_title('HWE p-values')
    else:
        ax3.text(0.5, 0.5, 'HWE p-values\nNot Available', ha='center', va='center', transform=ax3.transAxes)
    ...
    if ac_col in df.columns:
        if ac_col == 'AC':
            try:
                acs = df[ac_col].apply(lambda x: x[1] if isinstance(x, (list, np.ndarray)) and len(x) > 1 else x)
            except:
                acs = df[ac_col]
        else:
            acs = df[ac_col]
        ...
        if len(acs_filtered) > 0:
            ax4.hist(acs_filtered, bins=30, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
        ax4.set_xlabel('Allele Count')
        ax4.set_ylabel('Count')
        ax4.set_title('Allele Count')
        ax4.set_yscale('log')
    else:
        ax4.text(0.5, 0.5, 'Allele Count\nNot Available', ha='center', va='center', transform=ax4.transAxes)
    ...
    if available_cols:
        ...
        ax5.text(0.5, 0.5, 'Genotype Counts\nNot Available', ha='center', va='center', transform=ax5.transAxes)
    ...
    if 'n_not_called' in df.columns and 'n_called' in df.columns:
        ...
        if len(missing_rate) > 0:
            ax6.hist(missing_rate, bins=30, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
        ax6.set_xlabel('Missing Rate')
        ax6.set_ylabel('Count')
        ax6.set_title('Missing Data')
    else:
        ax6.text(0.5, 0.5, 'Missing Data\nNot Available', ha='center', va='center', transform=ax6.transAxes)

    # Add overall title
    fig.suptitle(f'Variant QC Overview (n={len(df)} variants)', fontsize=16, fontweight='bold')
...
def plot_qc_summary_dashboard(qc_results,
                            thresholds: Optional[Dict] = None,
                            style: str = 'default',
    ...
    **kwargs
    ...
    if hasattr(qc_results, 'get_sample_metrics_df'):
        ...
        raise ValueError("qc_results must be QCMetrics object or dictionary with 'sample' and 'variant' DataFrames")
    ...
    if sample_df is not None and not sample_df.empty:
        ...
        if 'call_rate' in sample_data.columns:
            ...
            ax1.hist(call_rates, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
            _add_threshold_lines(ax1, 'sample_call_rate', thresholds)
        ...
        if 'r_ti_tv' in sample_data.columns:
            ...
            q1, q99 = np.percentile(ti_tv, [1, 99])
            filtered_ti_tv = ti_tv[(ti_tv >= q1) & (ti_tv <= q99)]
            ax2.hist(filtered_ti_tv, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
            ax2.set_xlabel('Ti/Tv Ratio')
            ax2.set_ylabel('Count')
        ...
        if depth_col:
            ...
            ax3.hist(depths, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
            ax3.set_xlabel('Mean Depth')
            ax3.set_ylabel('Count')
        ...
        if available_cols:
            ...
            colors = [QC_COLORS['pass'], QC_COLORS['warn'], QC_COLORS['fail']]
            ax4.pie(genotype_data.values, labels=genotype_data.index, colors=colors[:len(genotype_data)], autopct='%1.0f%%')
    ...
    if variant_df is not None and not variant_df.empty:
        ...
        if 'call_rate' in variant_data.columns:
            ...
            ax5.hist(call_rates, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
        ...
        if af_col in variant_data.columns:
            if af_col == 'AF':
                try:
                    afs = variant_data[af_col].apply(lambda x: x[1] if isinstance(x, (list, np.ndarray)) and len(x) > 1 else x)
                except:
            ...
            afs_filtered = afs_filtered[(afs_filtered > 0) & (afs_filtered < 1)]
            if len(afs_filtered) > 0:
                ax6.hist(afs_filtered, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
        ...
        if 'p_value_hwe' in variant_data.columns:
            ...
            if len(pvals) > 0:
                log_pvals = -np.log10(pvals)
                ax7.hist(log_pvals, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
        ...
        if ac_col in variant_data.columns:
            if ac_col == 'AC':
                try:
                    acs = variant_data[ac_col].apply(lambda x: x[1] if isinstance(x, (list, np.ndarray)) and len(x) > 1 else x)
                except:
            ...
            acs_filtered = acs_filtered[acs_filtered > 0]
            if len(acs_filtered) > 0:
                ax8.hist(acs_filtered, bins=20, alpha=0.7, color=QC_COLORS['neutral'], edgecolor='black')
                ax8.set_yscale('log')

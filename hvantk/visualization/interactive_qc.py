...
def plot_interactive_sample_call_rates(sample_df: pd.DataFrame,
...
def plot_interactive_sample_titv(sample_df: pd.DataFrame,
                                thresholds: Optional[Dict] = None,
...
def plot_interactive_variant_call_rates(variant_df: pd.DataFrame,
                                       thresholds: Optional[Dict] = None,
...
def plot_interactive_allele_frequencies(variant_df: pd.DataFrame,
...
def plot_interactive_hwe_pvalues(variant_df: pd.DataFrame,
                               thresholds: Optional[Dict] = None,
...
def plot_interactive_sample_scatter(sample_df: pd.DataFrame,
    ...
    **kwargs
    ...
    if color_metric:
        fig = px.scatter(
            ...
            hover_data=[col for col in df.columns if col not in [x_metric, y_metric, color_metric]][:5],
            **kwargs
        )
    else:
        fig = px.scatter(
            ...
            x=x_metric,
            y=y_metric,
            hover_data=[col for col in df.columns if col not in [x_metric, y_metric]][:5],
            **kwargs
        )
...
def plot_interactive_qc_dashboard(qc_results,
                                title: str = "Interactive QC Dashboard",
                                **kwargs) -> go.Figure:
    """
    ...
    if variant_df is not None:
        ...
        if af_col in variant_data.columns:
            ...
            if len(afs_filtered) > 0:
                ...
                fig.update_yaxes(type="log", row=(plot_idx - 1) // cols + 1, col=(plot_idx - 1) % cols + 1)
...
def save_interactive_plot(fig: go.Figure,

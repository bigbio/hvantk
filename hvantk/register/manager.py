...
class RegistryManager:
    ...
    def update_validation_result(self, result: ValidationResult) -> None:
        ...
        logger.info(f"Updated validation result for {result.dataset_id}: {result.status.value}")
    ...
    def get_failed_datasets(self, limit: int = 10) -> list:
        ...
        failed_results = [
            (dataset_id, result) for dataset_id, result in self.registry.results.items()
            if result.status.value in ['tier1_failed', 'tier2_failed', 'tier3_failed', 'download_failed']
        ]

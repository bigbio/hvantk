...
class AnnotationStreamer(HailDataStreamer):
    ...
    def stream(self) -> Iterator[hl.Table]:
        ...
        """
        raise NotImplementedError("Annotation streamers typically process existing data chunks")
...
class VariantPredictionScoreStreamer(AnnotationStreamer):
    ...
    def annotate_chunk(self, chunk: hl.Table) -> hl.Table:
        ...
        self.logger.debug(f"Adding variant prediction scores to {chunk.count()} variants")
        ...
        annotated = annotated.annotate(
            ...
            .when(hl.is_defined(annotated.CADD_phred) & hl.is_defined(annotated.REVEL_score),
            ...
            .when(hl.is_defined(annotated.phyloP100way_vertebrate),
                  annotated.phyloP100way_vertebrate)
            .when(hl.is_defined(annotated.phastCons100way_vertebrate),
                  annotated.phastCons100way_vertebrate)
            .or_missing(),

            # Functional prediction consensus
            functional_consensus=hl.case()
            .when((annotated.SIFT_pred == "D") & (annotated.Polyphen2_HDIV_pred.contains("D")), "Damaging")
            .when((annotated.SIFT_pred == "T") & (annotated.Polyphen2_HDIV_pred.contains("B")), "Tolerated")
            .default("Unknown")
        )
...
class GeneExpressionStreamer(AnnotationStreamer):
    ...
    def __init__(self, expression_path: str, tissue_focus: str = "heart", chunk_size: int = 10000):
    ...
    def annotate_chunk(self, chunk: hl.Table) -> hl.Table:
        ...
        annotated = annotated.annotate(
            ...
            .when(hl.is_defined(annotated.heart_expression) & hl.is_defined(annotated.median_expression),
                  hl.log10(annotated.heart_expression / annotated.median_expression + 0.1))
            .or_missing(),
            ...
            .when(hl.is_defined(annotated.heart_expression) & (annotated.heart_expression > 5), True)
            .default(False)
        )
...
class GeneConstraintStreamer(AnnotationStreamer):
    ...
    def annotate_chunk(self, chunk: hl.Table) -> hl.Table:
        ...
        annotated = annotated.annotate(
            ...
            .when(hl.is_defined(annotated.oe_lof_upper) & (annotated.oe_lof_upper <= 0.35), "Intolerant")
            .when(hl.is_defined(annotated.oe_lof_upper) & (annotated.oe_lof_upper <= 0.65), "Moderate")
            .when(hl.is_defined(annotated.oe_lof_upper), "Tolerant")
            .default("Unknown"),

            # Combined constraint score
            constraint_score=hl.case()
            .when(hl.is_defined(annotated.pLI) & hl.is_defined(annotated.oe_lof_upper),
...
class PopulationFrequencyStreamer(AnnotationStreamer):
    ...
    def annotate_chunk(self, chunk: hl.Table) -> hl.Table:
        ...
        self.logger.debug(f"Adding population frequency data to {chunk.count()} variants")
        ...
        annotated = annotated.annotate(
            ...
            max_pop_af=hl.max(hl.array([
                annotated.AF_afr, annotated.AF_amr, annotated.AF_asj,
                annotated.AF_eas, annotated.AF_fin, annotated.AF_nfe,
                annotated.AF_oth, annotated.AF_sas

...
from hvantk.algorithms.annotation.annotate import (
    annotate_ccr,
    annotate_gevir,
...
def check_variant_tb(t: hl.Table, gene_col: str):
    ...
    if all([f in t.row for f in expected_fields]):
        ...
        sys.exit(
            f"Expected table with at least `locus`, `alleles` and {gene_col} fields."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-wf",
        "--write_to_file",
        help="Write output to BGZ-compressed file",
        action="store_true",
    )

...
- v4.0's dotted column names (`lof.pLI`, `lof.oe_ci.upper`, `mis.z_score`, …) are not valid Python identifiers once imported into the Hail Table — access them with bracket syntax (`ht['lof.pLI']`), not attribute syntax (`ht.lof.pLI`), per the note in `shared/constants.py`.

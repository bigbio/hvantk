...
The five profiled tissues are `Colon`, `Heart`, `Liver`, `Lung`, `Thyroid` (`FANG_TISSUES`, `hvantk/skills/pqtl/shared/constants.py`), which map to GTEx eQTL sub-tissue names via `FANG_TISSUE_EQTL_MAPPING` in the same file (e.g. `Colon` → `Colon_Transverse`, `Colon_Sigmoid`; `Heart` → `Heart_Left_Ventricle`, `Heart_Atrial_Appendage`) — used to align pQTL and eQTL tissue granularity for the cascade join.

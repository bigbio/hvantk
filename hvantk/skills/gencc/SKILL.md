...
- GenCC rate-limits: a scheduled drift regeneration on 2026-08-04 got back HTTP 429 and failed the run outright. The drift probe uses `request_with_retry` (`hvantk/core/utils/http.py`) instead of bare `requests` for its HEAD and GET calls — it retries transient statuses (429 plus the 5xx family) and connection errors with exponential backoff, honouring `Retry-After` but clamped to a max sleep so a long backoff can't stall CI.

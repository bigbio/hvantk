...
4. **Build** via `hvantk reprocess alphagenome:predictions --raw-dir <path/to/variants.ht-or-tsv> --skip-download --output <out.ht> --plugin-arg config_path=<path/to/config.yaml> [--plugin-arg no_resume=true]`.
   Two non-obvious parts, both forced by this plugin declaring neither `lifecycle.download` nor `lifecycle.parse`: `--skip-download` is **required** (without it `reprocess_cli.py` raises `UsageError: alphagenome:predictions has no lifecycle.download declared`), and `--raw-dir` must point at the **input file itself**, not a directory — with no parse stage the runner forwards the path straight to the builder, and a directory reaches `open()` and raises `IsADirectoryError`.

...
4. **Build** via `hvantk reprocess alphagenome:predictions --raw-dir <path/to/variants.ht-or-tsv> --output <out.ht> --plugin-arg config_path=<path/to/config.yaml> [--plugin-arg no_resume=true]`.
   One non-obvious part, forced by this plugin declaring no `lifecycle.parse`: `--raw-dir` must point at the **input file itself**, not a directory — with no parse stage the runner forwards the path straight to the builder, and a directory reaches `open()` and raises `IsADirectoryError`.

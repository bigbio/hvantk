...
class PipelineState:
    ...
    def load(cls, path: Path) -> 'PipelineState':
        ...
        state = cls(
            config=config,
            current_stage=PipelineStage(state_dict['current_stage']) if state_dict['current_stage'] else None,
    ...
    def mark_stage_complete(self, stage: PipelineStage, output_path: Optional[str] = None):
...
class PipelineRunner:
    ...
    def _setup_output_paths(self):
        ...
        self.paths = {
            ...
            'log': str(logs_dir / f'pipeline_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
        }
    ...
    def show_plan(self):
        ...
        if not self.config.skip_combine_gvcfs:
            ...
            print(f"  [SKIP] Combine gVCFs (using existing VDS: {self.config.vds_path})")
        ...
        if not self.config.skip_export_pvcf:
            ...
            if self.config.apply_qc_filters:
                ...
                print(f"        Min sample call rate: {self.config.min_sample_call_rate}")
                print(f"        Min variant call rate: {self.config.min_variant_call_rate}")
            stage_num += 1
        else:
    ...
    def _run_compute_variant_qc(self) -> str:
        ...
        self.logger.info(f"   ✓ Variant QC CSV (sample): {self.paths['variant_qc_csv']}")
        ...
        if self.config.apply_qc_filters:
            ...
            if not self.config.skip_compute_sample_qc:
                mt = filter_samples_by_qc(
                    mt,
                    min_call_rate=self.config.min_sample_call_rate
                )
                self.logger.info(f"   ✓ Filtered samples by call rate >= {self.config.min_sample_call_rate}")

            # Filter variants
            if not self.config.skip_compute_variant_qc:
                mt = filter_variants_by_qc(
                    mt,
                    min_call_rate=self.config.min_variant_call_rate
                )
                self.logger.info(f"   ✓ Filtered variants by call rate >= {self.config.min_variant_call_rate}")

            # Save filtered MT
            filtered_mt_path = str(Path(self.paths['mt']).parent / f"{self.config.output_prefix}_filtered.mt")

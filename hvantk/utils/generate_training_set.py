...
CHD_LABEL_CLINVAR = ["Congenital_heart_disease", "Congenital_heart_defect"]
...
clinvar_ht = clinvar_ht.filter(
    ~clinvar_ht.Consequence.any(lambda x: x == "synonymous_variant")
)
...
ts_ann_expr = {
    "is_tp_site": hl.case()
    .when(
        clinvar_ht.info.CLNSIG.any(
            lambda x: hl.set(PATHOGENIC_LABEL_CLINVAR).contains(x)
        )
        & hl.set(chd_gene_set).contains(clinvar_ht.gene),
        True,
    )
    .when(
        clinvar_ht.info.CLNDN.any(lambda x: hl.set(CHD_LABEL_CLINVAR).contains(x)), True
    )
    .default(False),
    "is_tn_site": hl.case()
    .when(
        clinvar_ht.info.CLNSIG.any(lambda x: hl.set(BENIGN_LABEL_CLINVAR).contains(x)),
        True,
    )
    .default(False),
}

...
class SchemaValidator:
    ...
    def __init__(self, schema_dir: Optional[Path] = None):
        self.schema_dir = (
            schema_dir or Path(__file__).parent.parent / "resources" / "schemas"
        )
        self.schemas = {}
        self._load_schemas()
    ...
    def _load_schemas(self):
        ...
        try:
            for schema_file in self.schema_dir.rglob("*.json"):
                try:
                    ...
                    logger.info(
                        f"Loaded schema: {schema_name} from {schema_file.relative_to(self.schema_dir)}"
                    )
        ...
        if errors:
            logger.warning(
                f"Encountered {len(errors)} errors while loading schemas. Successfully loaded: {list(self.schemas.keys())}"
            )
    ...
    def validate_dataset(
        self, dataset_metadata: Dict, schema_type: str
    ) -> Tuple[bool, List[str]]:
        """
        ...
        if schema_type not in self.schemas:
            return False, [
                f"Schema '{schema_type}' not found. Available: {list(self.schemas.keys())}"
            ]
    ...
    def suggest_schema_type(self, dataset_metadata: Dict) -> str:
        ...
        if any(key in dataset_metadata for key in ["expression_unit", "platform_type"]):
            ...
            key in dataset_metadata
            for key in ["quantification_method", "protein_inference_method"]
        ):

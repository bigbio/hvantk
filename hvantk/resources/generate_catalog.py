...
def generate_master_catalog():
    ...
    for omics_type in omics_types:
        ...
        if datasets_file.exists():
            ...
            catalog["datasets"][omics_type] = {
                ...
                "summary": {
                    "data_sources": list(set(d.get("data_source", "Unknown") for d in datasets)),
                    "organisms": list(set(d.get("organism", "Unknown") for d in datasets)),
                    "sample_range": {
                        "min": min((d.get("sample_count", 0) for d in datasets), default=0),
                        "max": max((d.get("sample_count", 0) for d in datasets), default=0)
                    }
                }
            }
    ...
    catalog["summary"] = {
        ...
        "omics_types": len([t for t in omics_types if catalog["datasets"][t]["count"] > 0])
    }
